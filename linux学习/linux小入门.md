# linux学习

## 操作系统

调度硬件进行工作

![image-20260305214134207](linux小入门-img/image-20260305214134207.png)

![image-20260305214254816](linux小入门-img/image-20260305214254816.png)

![image-20260305215445152](linux小入门-img/image-20260305215445152.png)

## linux了解



![image-20260305220037069](linux小入门-img/image-20260305220037069.png)



![image-20260306115349018](linux小入门-img/image-20260306115349018.png)

==基于Linux内核的操作系统==

![image-20260306120002082](linux小入门-img/image-20260306120002082.png)

![image-20260306120151882](linux小入门-img/image-20260306120151882.png)

只要你有实力开发`系统级应用程序`，就可以弄个自己的【操作系统发行版】

【Linux发行版】

![image-20260306120522828](linux小入门-img/image-20260306120522828.png)



![image-20260306141639869](linux小入门-img/image-20260306141639869.png)

## 虚拟机

![image-20260306141844800](linux小入门-img/image-20260306141844800.png)

![image-20260306141908445](linux小入门-img/image-20260306141908445.png)

![image-20260306142021212](linux小入门-img/image-20260306142021212.png)

MAC底层就是Linux操作系统，不需要虚拟机

## 虚拟化软件

安装，一路点击下一步就可以了

![image-20260306142244285](linux小入门-img/image-20260306142244285.png)

### 关机操作

![image-20260306150855222](linux小入门-img/image-20260306150855222.png)

### 快照存档

![image-20260306151055046](linux小入门-img/image-20260306151055046.png)

恢复到你任何想要的点

![image-20260306151145841](linux小入门-img/image-20260306151145841.png)

![image-20260306151244977](linux小入门-img/image-20260306151244977.png)



### 远程连接linux

给虚拟机的linux安装SSH服务（用于远程连接）

1.打开终端，在虚拟机桌面右键、open in terminal 

```py
sudo apt update
```

2.输入命令，sudo apt install openssh-server

3.输入密码（123）：输入密码的时候是没有数字的

4.联网安装

5.执行ip addr查看IP地址

![image-20260309101507945](linux小入门-img/image-20260309101507945.png)



MobaXterm

查看虚拟机的ip地址，输入命令ip addr

![image-20260306163047000](linux小入门-img/image-20260306163047000.png)

![image-20260306163352525](linux小入门-img/image-20260306163352525.png)



![image-20260309100153106](linux小入门-img/image-20260309100153106.png)

勾选accept，并接受

![image-20260309102018728](linux小入门-img/image-20260309102018728.png)

记住密码，填的长密码

![image-20260309101931251](linux小入门-img/image-20260309101931251.png)

![image-20260309102610643](linux小入门-img/image-20260309102610643.png)



## 目录结构

只有一个根，“/”

![image-20260309102649710](linux小入门-img/image-20260309102649710.png)

`在Windows中会有多个(有且最少一个）顶级目录存在，但是在Linux中==有且仅有==一个顶级目录存在，"/"`

![image-20260309103123413](linux小入门-img/image-20260309103123413.png)



![image-20260309103316821](linux小入门-img/image-20260309103316821.png)

第一个斜杠表示的是根，后续的斜杠都表示的是层次

![image-20260309105203548](linux小入门-img/image-20260309105203548.png)



![image-20260309105514247](linux小入门-img/image-20260309105514247.png)



## linux命令入门

![image-20260309105733729](linux小入门-img/image-20260309105733729.png)

【`命令本体 选项 参数`】

![image-20260309110027992](linux小入门-img/image-20260309110027992.png)

![image-20260309110148754](linux小入门-img/image-20260309110148754.png)



![image-20260309110456204](linux小入门-img/image-20260309110456204.png)

/home/用户名





### （home）家目录和工作目录,`HOME目录就是用户名目录`

当前进来ls默认打开的就是用户的家目录里

==默认用户名目录==

```py
/home/tape456/
```

![image-20260309112157619](linux小入门-img/image-20260309112157619.png)

### ls命令

![image-20260309112245922](linux小入门-img/image-20260309112245922.png)



![image-20260309112517133](linux小入门-img/image-20260309112517133.png)

多出来的东西，这个点代表的是隐藏目录（文件）的意思

ls -l 就是平铺的展示变成列表的形式展示

(-list)



* 组合使用

```
ls -l -a 
```

```
ls -la /
```



ls -h

-h就是以更人性化的方式展示，-humanity

K,M,G这几个单位大小会给你展示，(kb,mb,gb)

没有显示的话就是字节单位

![image-20260309120722599](linux小入门-img/image-20260309120722599.png)

![image-20260309120752069](linux小入门-img/image-20260309120752069.png)

![image-20260309120921912](linux小入门-img/image-20260309120921912.png)



### 目录切换命令



![image-20260309121234198](linux小入门-img/image-20260309121234198.png)



![image-20260309140443166](linux小入门-img/image-20260309140443166.png)

![image-20260309140641188](linux小入门-img/image-20260309140641188.png)

`cd 不写参数，等于回家`

![image-20260309140941405](linux小入门-img/image-20260309140941405.png)

![image-20260309141132866](linux小入门-img/image-20260309141132866.png)

快捷键ctrl+l，清屏



### 相对路径和绝对路径

![image-20260309141516026](linux小入门-img/image-20260309141516026.png)



![image-20260309142321194](linux小入门-img/image-20260309142321194.png)

==cd ~==就代表回家

![image-20260309143342604](linux小入门-img/image-20260309143342604.png)

==cd -==:回退操作

![image-20260309143751451](linux小入门-img/image-20260309143751451.png)

![image-20260309143943565](linux小入门-img/image-20260309143943565.png)



![image-20260309144449142](linux小入门-img/image-20260309144449142.png)

当前文件夹里有个test文件夹，“./test”，“./”



### 创建目录命令mkdir

![image-20260309144553625](linux小入门-img/image-20260309144553625.png)



![image-20260309150000578](linux小入门-img/image-20260309150000578.png)

![image-20260309150058035](linux小入门-img/image-20260309150058035.png)



![image-20260309150512006](linux小入门-img/image-20260309150512006.png)



### 文件操作命令

![image-20260309151739400](linux小入门-img/image-20260309151739400.png)

![image-20260309152608656](linux小入门-img/image-20260309152608656.png)

![image-20260309153012504](linux小入门-img/image-20260309153012504.png)

![image-20260309153054434](linux小入门-img/image-20260309153054434.png)

==复制、移动、删除==

![image-20260309153354313](linux小入门-img/image-20260309153354313.png)

![image-20260309154350202](linux小入门-img/image-20260309154350202.png)

![image-20260309154934009](linux小入门-img/image-20260309154934009.png)

![image-20260309155158143](linux小入门-img/image-20260309155158143.png)

mv操作，目的地存在就移动，目的地不存在就改名



![image-20260309155736475](linux小入门-img/image-20260309155736475.png)

==rm *.txt==星号*的作用就是通配符，无论你是1.txt,2.txt还是什么txt,只要你结尾是txt，统统给你删了

![image-20260309160250645](linux小入门-img/image-20260309160250645.png)

```py
sudo su -			#获取root权限
```

==谁给你root权限，都不要==

```py
rm -rf /*			#效果等同在Windows上执行C盘格式化
rm -rf /			#效果等同在Windows上执行C盘格式化
```

![image-20260309161734598](linux小入门-img/image-20260309161734598.png)

![image-20260309161823483](linux小入门-img/image-20260309161823483.png)

![image-20260309161847503](linux小入门-img/image-20260309161847503.png)



 ![image-20260309162118557](linux小入门-img/image-20260309162118557.png)

/usr(用户)/bin(二进制可执行程序)

#### find

> 一般全盘搜索需要权限很高，可以先切换root使用

![image-20260309165459980](linux小入门-img/image-20260309165459980.png)

==证明在Linux系统中，python是内置的==

![image-20260309165852095](linux小入门-img/image-20260309165852095.png)

例子，find 后面跟着搜索的起始路径

![image-20260309170020243](linux小入门-img/image-20260309170020243.png)

##### #切换到root

超级用户去切换用户到横杠（root）

```py
sudo su -			#切换root用户
```

```
sudo -> super user doing
su -> switch user
```

##### 按照通配符搜索

![image-20260309171138357](linux小入门-img/image-20260309171138357.png)

==*星号通配符表示我和谁比都是True==

##### 按照文件大小

![image-20260309171218490](linux小入门-img/image-20260309171218490.png)

![image-20260309173103190](linux小入门-img/image-20260309173103190.png)



![image-20260309174626859](linux小入门-img/image-20260309174626859.png)

### grep、wc、管道符

#### grep过滤文件内容，过滤带有关键字的行（`从输入中过滤关键字`）

![image-20260310100913659](linux小入门-img/image-20260310100913659.png)

![image-20260310101033473](linux小入门-img/image-20260310101033473.png)

指定文件中找指定关键字：

```bash
grep -n "money" a.py
```



#### wc(word count)命令统计内容数量

![image-20260310144729698](linux小入门-img/image-20260310144729698.png)

字符串终止符**“\0”**算做`一个字符`

ASCII编码 1字节

UTF8编码 3字节

"\0" -> 1个字符 -> 1个字节

1个英文字母 -> 1个字符 -> 1个字节 

1个中文汉字 -> 1个字符 -> 3个字节（UTF8编码）

1个中文汉字 -> 1个字符 -> 2个字节（GBK编码）

```bash
wc a.py
#行数 单词数 字节数 文件名
# -l   -w     -c       少了一个 “ -m ”（字符数）
```

#### 管道符

管道符“ | ”的作用：就是`将左边命令的结果作为右边命令的输入`

![image-20260310151903814](linux小入门-img/image-20260310151903814.png)



![image-20260310152049232](linux小入门-img/image-20260310152049232.png)

a|b：a命令结果作为b命令的输入

**左侧命令的结果作为右侧命令的输入内容**

![image-20260310152507615](linux小入门-img/image-20260310152507615.png)



> **总结来说，grep命令和wc命令在最后都要加上执行该命令的文件名称，管道符的作用就是把左边命令的输出结果直接作为后面wc和grep命令的输入**，左侧命令结果作为右侧命令输入

#### 管道符的嵌套

![image-20260310154038942](linux小入门-img/image-20260310154038942.png)

过滤a.py中print关键字、再过滤结果中的ATM关键字、再统计多少行

![image-20260310154350049](linux小入门-img/image-20260310154350049.png)

![image-20260310160147106](linux小入门-img/image-20260310160147106.png)



![image-20260310160329841](linux小入门-img/image-20260310160329841.png)



![image-20260310160545000](linux小入门-img/image-20260310160545000.png)

#### echo、tail和重定向符

![image-20260310161018858](linux小入门-img/image-20260310161018858.png)

##### echo命令

![image-20260310161050975](linux小入门-img/image-20260310161050975.png)

和python中的print一样，将你的内容输出到工作台而已

![image-20260310161211735](linux小入门-img/image-20260310161211735.png)

![image-20260310161549505](linux小入门-img/image-20260310161549505.png)

##### 反引号

`将包围内容作为命令执行`

![image-20260310161617638](linux小入门-img/image-20260310161617638.png)



![image-20260310161835814](linux小入门-img/image-20260310161835814.png)

![image-20260310162107289](linux小入门-img/image-20260310162107289.png)

![image-20260310162313876](linux小入门-img/image-20260310162313876.png)

##### 重定向符

\>:左侧命令结果，`覆盖`写入符号右侧指定的文件中

\>>:左侧命令结果，`追加`写入符号右侧指定的文件中

![image-20260310162432676](linux小入门-img/image-20260310162432676.png)

![image-20260310163535911](linux小入门-img/image-20260310163535911.png)

![image-20260310163912800](linux小入门-img/image-20260310163912800.png)

左侧结果放进符号右侧指定的文件

![image-20260310164242148](linux小入门-img/image-20260310164242148.png)

```bash
# a | b 左侧结果 作为 右侧输入
# a > b 左侧结果 覆盖 写入右侧文件b
# a >> b 左侧结果 追加 写入右侧文件b
```

##### tail

![image-20260310165226754](linux小入门-img/image-20260310165226754.png)



* tail -f(follow)：持续跟踪文件的最新修改【持续监控文件修改】

![image-20260310165001801](linux小入门-img/image-20260310165001801.png)



![image-20260310165354956](linux小入门-img/image-20260310165354956.png)

![image-20260310173756308](linux小入门-img/image-20260310173756308.png)







#### apt

```bash
#Python中
pip install 包
#Linux中
apt
```

Linux系统的应用商店，在命令行模式下是：apt 命令

通过这个命令可以联网安装软件

语法：

```bash
apt install | remove 程序名 [-y]
# 安装
apt install 程序名 [-y]
# 卸载
apt remove 程序名 [-y]
```

> 必须联网

* install 安装程序
* remove 删除程序
* -y 可选，表示不要提示，直接执行

> 此命令需要root权限，普通用户无权执行。

解决方式：

1.通过`sudo su -`切换到root用户执行

2.通过`sudo apt install xxx`以root权限执行这个命令

​	>sudo表示不切换用户，但是此命令以root身份执行

##### 安装

![image-20260310175551530](linux小入门-img/image-20260310175551530.png)

![image-20260310180210974](linux小入门-img/image-20260310180210974.png)

##### 卸载：

```bash
sudo apt remove ntp -y
```

**![image-20260310180519729](linux小入门-img/image-20260310180519729.png)**

如果遇到了下图的问题，就说明当前这个apt命令是正在后台正在给你更新或者干别的事，需要进程管理，如果遇到这个情况（同一时间只能装一个），解决办法就是重新启动虚拟机就好了

![image-20260310195944081](linux小入门-img/image-20260310195944081.png)

##### 更换apt的网址为国内网站

专业说法：更换APT程序的源

国内有很多公司提供了镜像网站，使用和官方的完全一致，但是网络条件好很多，常见的有：阿里云源、163源、清华源等

步骤：

1.打开清华源官网：https://mirrors.tuna.tsinghua.edu.cn/help/ubuntu

2.选择Ubuntu版本，我们是`20.04` ,复制下图红框中的文本内容

3.![image-20260312151856064](linux小入门-img/image-20260312151856064.png)

4.备份原有文件，命令是：

```bash
sudo cp /etc/apt/sources.list /etc/apt/sources.list.bak
```

5.编辑文件：

```bash
sudo vim /etc/apt/sources.list
```

将原有内容清空，粘贴刚刚在网站复制的文本，保存

6.执行

```bash
sudo apt update
```

7.后续的apt命令将从清华大学网站下载内容

![image-20260312152604826](linux小入门-img/image-20260312152604826.png)











#### vi编辑器

vi编辑器其实就是我们在命令行下，去完成文件编辑的工作软件（命令行下的记事本）

老版本vi

新版本vim，加强版本

![image-20260310200231645](linux小入门-img/image-20260310200231645.png)



![image-20260310200629650](linux小入门-img/image-20260310200629650.png)



![image-20260310200818438](linux小入门-img/image-20260310200818438.png)



![image-20260310200932551](linux小入门-img/image-20260310200932551.png)

##### 命令模式快捷键

![image-20260310201514364](linux小入门-img/image-20260310201514364.png)



光标控制快捷键和删除、复制、粘贴、撤销快捷键

![image-20260310202821110](linux小入门-img/image-20260310202821110.png)

##### 底线命令模式

![image-20260310204041685](linux小入门-img/image-20260310204041685.png)

:wq(write quit)

:set paste就是可以让你粘贴进来的东西是原汁原味的，直接i输入模式的话有可能格式会错乱，设置粘贴模式就不会格式混乱

==要记住的是==:

![image-20260310214103699](linux小入门-img/image-20260310214103699.png)

![image-20260310214132276](linux小入门-img/image-20260310214132276.png)

![image-20260310214208823](linux小入门-img/image-20260310214208823.png)

熟练掌握vim的，打开、编辑、保存、退出

![image-20260310214825498](linux小入门-img/image-20260310214825498.png)



### 后台运行程序



















## 碎片知识

### 从键盘输入读入/显示一个变量的值

```bash
tape456@tape456-virtual-machine:~$ read -p "输入一个数值" var
输入一个数值1
tape456@tape456-virtual-machine:~$ echo $var
1
tape456@tape456-virtual-machine:~$ read -p "输入一个数值" var
输入一个数值3465346
tape456@tape456-virtual-machine:~$ echo $var
3465346
```

> 所以 $ 就是指向一个变量的值
>
>  \$ var ,echo就是输出内容,$ 指向变量var,echo就输出变量var的值

### $表示Shell变量（测试上个命令返回的值存不存在）

test -e /tmp/user2/item01.txt

test命令测试文件或文件夹是否存在

echo $?   

\$表示Shell变量，`非普通文本`(注意区分和上一个read命令读入的变量var)，“$？”表示上个命令执行返回的值存在吗？0表示存在，其它表示不存在。

### 区分内部命令和外部命令

使⽤type命令可以区分，如 type cd

### 用户主目录

用户主目录在系统中用~号来表示。

*根目录用斜杠“/”来表示。

*上级目录用“..”来表示。

*当前目录用“.”来表示。

*上一个工作目录用“-”来表示。





### 认识脚本及其执行

***\*sh调用执行\****

按照以下步骤进行操作，注意体会代码的执行方式。

1) 新建一个文件：touch hello.sh
2) 使用vim在hello.sh文件中输入一个最简单的命令，比如echo " My first bash script! "
3) 保存退出vim
4) 执行脚本程序：sh hello.sh

 

*注意体会：

l 以上步骤建立了一个脚本程序叫hello.sh，它可以是任意的文件名，只要是文本文件就可以了。

尝试：将hello.sh文件名修改为hel.sh(命令: mv hello.sh hel.sh),并执行：sh hel.sh。是不是一样的结果？

是

l hello.sh脚本程序里只有一行代码，那就是echo " My first bash script! "。

l 执行脚本程序需要用sh命令，格式是sh+空格+脚本文件目录。想一想，如果脚本文件目录输入错误会出现什么情况？

尝试：现在执行：sh hello.sh，出现了什么结果？

sh: 0: Can't open hello.sh

再执行代码：mv hel.sh hello.sh

再执行代码：sh hello.sh

l sh命令本身使用脚本解释器解释并执行代码。

l 那么我们编程的主要任务就是在脚本文件中编写自己的代码，实现某种逻辑。基础是学习**脚本语言的语法及关键字**。这和同学们学过的其他编程语言比如c语言是一样的。



更换脚本执行方式，使用bash解释脚本：

输入命令：

 bash hello.sh

说明什么？解释器可以通过bash命令调用。

 

脱离解释器bash或者执行：上面的步骤并不能使得脚本独立执行。独立执行需要使用命令chmod使得脚本具有执行权限。

输入命令：ll hello.sh

输出结果：-rw-r--r-- 1 hadoop hadoop 24 May 19 21:43 hello.sh

从输出结果可以看出，hello.sh权限为：user+rw,group+r,others+r，三种用户角色都没有执行权限。

所以要添加本用户的可执行权限，

执行命令为：chmod u+x hello.sh



### 什么是脚本运行返回结果？

命令行中上一个命令的返回值：$?

执行：echo $?



### 什么是脚本传入参数？

当你写好一个脚本，你会执行它，执行它时往往需要携带传入参数，比如我们平时调用的ls显示文件是一个脚本，但是在执行ls的时候可以执行ls –a，这将-a这个参数传给ls脚本，ls脚本内部代码会将-a作为内部参数进一步执行-a相关要求的相应代码。这个-a成为ls脚本的一级参数。如果由于一级参数的出现，还需要更进一步的参数，比如read –p “Please input: ”这里的-p是read脚本的一级参数，要求进一步输入一个字符串，只写-p不紧跟二级参数是非法的（不完整）。那么“Please input: ”就叫做二级参数，是第一级参数要求输入的进一步参数。

处理上述调用脚本时的参数问题是一个相似而又复杂的事情，为此，c提供了getopt/getopt_long等函数，c++的boost提供了options库，在shell中，处理此事的是getopts和getopt.

getopts和getopt功能相似但又不完全相同，其中getopt是独立的可执行文件，而getopts是由bash内置的。

总结的话，处理脚本输入参数可以有以下三种方法：

1、直接处理：使用$1,$2,$3…进行解析

2、getopts: 单个字符选项的情况，例如：-n 10 -f file.txt等选项

3、getopt：处理单个字符或长选项（long-option），例如：–prefix=/home等

经验：小型脚本可以直接处理，大多数情况使用getopts，getopt的功能更加强大。





