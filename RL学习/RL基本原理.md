...
在下面的 MAB 算法基本框架中，我们将**根据策略选择动作**、**根据动作获取奖励**和**更新期望奖励估值**放在 `run_one_step()` 函数中，由每个继承 Solver 类的策略具体实现。而**更新累积懊悔和计数**则直接放在主循环 `run()` 中。
...
```py
旧写法（Python 2 风格，但 Python 3 也支持）
...
![image-20260422222532521](RL基本原理-img/image-20260422222532521.png)
...
==我们将用于拟合函数Q函数的神经网络称为**Q 网络**，如图 7-2 所示。==
...
![image-20260205161138585](RL基本原理-img/image-20260205161138585.png)

#### 1. 整体含义

我们要找到**最优的网络参数** $$\omega^*$$，让网络预测的 $$Q_\omega(s_i,a_i)$$ 尽可能接近TD目标（标准答案），用**均方误差（MSE）**来衡量这个“接近程度”，然后让这个误差最小。

#### 2. 拆成每一部分看

##### ① $$\omega^* = \arg\min_\omega [\dots]$$

- $$\arg\min_\omega$$：找到一个 $$\omega$$（网络的所有参数），让后面方括号里的**数值最小**。
- 大白话：找到“让网络错得最少”的那套参数，也就是让网络的预测最接近标准答案的方法。

##### ② $$\frac{1}{2N} \sum_{i=1}^N$$

- $$\sum_{i=1}^N$$：对**N条训练数据**（也就是经验回放池里的N条经验 $$(s_i,a_i,r_i,s_i'$$）求和。
- 1/N：对N条数据的误差求**平均**，避免数据越多，误差总和越大，方便训练稳定。
- 1/2：数学小技巧！平方项求导后会多出一个系数2，乘1/2后导数就变成 $$Q_\omega - \text{TD目标}$$，计算更方便，不影响“最小化误差”的本质。

##### ③ $$\left[ Q_\omega(s_i,a_i) - \left( r_i + \gamma \max_{a'} Q_\omega(s_i',a') \right) \right]^2$$

这是公式的**核心**，也就是**均方误差（MSE）**：

- $$Q_\omega(s_i,a_i$$：当前网络对“状态 $$s_i$$ 选动作 $$a_i$$”的**预测Q值**（你做的题）。
- $$r_i + \gamma \max_{a'} Q_\omega(s_i',a')$$：TD目标（老师给的标准答案）。
- $$(\text{预测值} - \text{目标值})^2$$：预测和标准答案的**差距的平方**。
  - 平方的好处：不管差是正还是负，都变成正数，不会相互抵消；差距越大，惩罚越重（比如差1分，平方是1；差3分，平方是9，会更严厉地惩罚大错误）。
...
<img src="RL基本原理-img/image-20260508175135027.png" alt="image-20260508175135027" style="zoom:67%;" />



<img src="RL基本原理-img/image-20260508175427436.png" alt="image-20260508175427436" style="zoom:67%;" />
...
我们可以发现，与普通的 DQN 相比，Double DQN 比较少出现值大于 0 的情况，说明值过高估计的问题得到了很大缓解。
...
**基于价值的方法**包括：Q-learning（处理有限状态）、DQN（解决连续状态） 及 DQN 改进算法，目标学习值函数，`根据值函数导出一个策略`，学习过程中不存在一个显式的策略

**基于策略的方法**：直接显式地学习一个目标策略。**策略梯度**是基于策略的方法的基础
...
目标是要寻找一个**最优策略**并最大化这个策略**在环境中**的**期望回报**,将策略学习的目标函数定义为:

![image-20260526143721209](RL基本原理-img/image-20260526143721209.png)

策略Π下的状态访问分布,用![image-20260526143854160](RL基本原理-img/image-20260526143854160.png)表示

对目标函数求梯度，可以得到如下式子

![image-20260526143922818](RL基本原理-img/image-20260526143922818.png)

*策略梯度算法*为**在线策略（on-policy）算法**

![image-20260526144318538](RL基本原理-img/image-20260526144318538.png)



![image-20260526152634283](RL基本原理-img/image-20260526152634283.png)
...
在==策略梯度的基础上==,对**Q的估计**用来==蒙特卡洛方法==来估计

![image-20260526153131152](RL基本原理-img/image-20260526153131152.png)



![image-20260526153641904](RL基本原理-img/image-20260526153641904.png)



最小化损失函数，就是最大化后面的目标函数的梯度值，沿着目标函数的梯度上升的方向，就能最大化这个策略**在环境中**的**期望回报**

![image-20260526201710881](RL基本原理-img/image-20260526201710881.png)

REINFORCE 算法理论上是能保证局部最优的，它实际上是借助蒙特卡洛方法采样轨迹来估计动作价值，这种做法的一大优点是可以得到无偏的梯度。但是，**正是因为使用了蒙特卡洛方法，REINFORCE 算法的梯度估计的方差很大**，可能会造成`一定程度上的不稳定`，这也是第 10 章将介绍的 `Actor-Critic 算法`要解决的问题。

小结里公式推导过程的问题：

![image-20260526215547819](RL基本原理-img/image-20260526215547819.png)





### TRPO算法,**信任区域策略优化**（trust region policy optimization，TRPO）

基于策略的算法虽然简单直观，但是应用过程中会遇到==训练不稳定==的情况。

基于策略的方法：**参数化**智能体的策略，并**设计**衡量策略好坏的**目标函数**，通过梯度上升的方法来**最大化**这个**目标函数**，使得策略最优。

TRPO 使用了**库尔贝克-莱布勒**（Kullback-Leibler，KL）散度来衡量策略之间的距离:

![image-20260528111649169](RL基本原理-img/image-20260528111649169.png)

这里的不等式约束定义了策略空间中的一个 KL 球，被称为信任区域。在这个区域中，可以认为当前学习策略和环境交互的状态分布与上一轮策略最后采样的状态分布一致，进而可以基于一步行动的重要性采样方法使当前学习策略稳定提升。

![image-20260528113545405](RL基本原理-img/image-20260528113545405.png)

#### 广义优势估计

![image-20260603154756066](RL基本原理-img/image-20260603154756066.png)
...
![image-20260604110537898](RL基本原理-img/image-20260604110537898.png)

TRPO 使用泰勒展开近似、共轭梯度、线性搜索等方法直接求解。`PPO 的优化目标与 TRPO 相同`，但 PPO 用了一些相对简单的方法来求解。
...
![image-20260604152638155](RL基本原理-img/image-20260604152638155.png)

红点位置：\(r=1\)（新 = 旧，没更新）

- 左图\(A>0\)：r从 1 往右涨到\(1+\varepsilon\)后，曲线走平，不再提升概率
- 右图\(A<0\)：r从 1 往左跌到\(1-\varepsilon\)后，曲线走平，不再压低概率

一句话总结

> 好动作不能无脑暴涨概率（上限\(1+epsilon\)），坏动作不能无脑暴跌概率（下限\(1-epsilon\)），用截断锁住新旧策略差距，稳定离线策略更新。
...
用类似的思想来处理动作空间无限的环境并且使用的是离线策略算法呢？

![image-20260605150845749](RL基本原理-img/image-20260605150845749.png)
...
**强化学习里的 “采样效率”，核心指的就是「数据的利用效率」，也就是 “用多少环境交互样本，能把模型训练好”**。

Soft Q-learning 不存在一个显式的策略函数，而是使用一个函数的波尔兹曼分布，在连续空间下求解非常麻烦。于是 SAC 提出使用一个 Actor 表示策略函数，从而解决这个问题。

![image-20260608112301060](RL基本原理-img/image-20260608112301060.png)

**横轴**：代表**动作空间**（也就是智能体可以选择的所有动作）。

**纵轴**：代表**价值 / 概率高低**，越高就代表 “这个动作越好、被选中的概率越高”。

**青色曲线 Q**：是**Q 值函数**，代表每个动作的预期收益（哪个动作收益高，曲线就越高）。

**红色 / 黄色曲线 π**：是**策略函数**，代表智能体选每个动作的概率分布。

图里的 “高低” 是**短期的、当前估计的收益**，而长期最优解，往往藏在这些短期收益不高、但有探索价值的次高峰里。SAC 的熵正则化，就是给了智能体一个 “不放弃探索” 的理由，让它有机会发现这些真正的最优解。



对连续动作空间的环境，SAC 算法的策略输出高斯分布的均值和标准差，但是根据高斯分布来采样动作的过程是不可导的。因此，我们需要用到**重参数化技巧**（reparameterization trick）
...
![image-20260608224543612](RL基本原理-img/image-20260608224543612.png)

我们可以把它整理一下：

![image-20260608224552252](RL基本原理-img/image-20260608224552252.png)

![image-20260608224613235](RL基本原理-img/image-20260608224613235.png)

在 SAC 中，我们的目标是**最小化**![image-20260608224628998](RL基本原理-img/image-20260608224628998.png)，所以我们会对![image-20260608224637753](RL基本原理-img/image-20260608224637753.png)求梯度并更新：

![image-20260608224647387](RL基本原理-img/image-20260608224647387.png)
...
<img src="RL基本原理-img/image-20260608224718410.png" alt="image-20260608224718410"  />

> 

<img src="RL基本原理-img/image-20260608224736718.png" alt="image-20260608224736718" style="zoom: 80%;" />



<img src="RL基本原理-img/image-20260616112802960.png" alt="image-20260616112802960" style="zoom:50%;" />

结合 SAC 算法原理：**公式少打了一层外层括号**，正确形式是 \(\gamma \cdot \big(\min Q^- - \alpha\log\pi\big)\)
...
![image-20260626161911423](RL基本原理-img/image-20260626161911423.png)

1. **离散动作（比如游戏按键：上、下、左、右、开火，有限个可选动作）**

损失用**最大似然估计（交叉熵损失）**

- 场景：动作是分类问题，一共 N 个可选动作
- 网络输出：每个动作的概率（比如 0.8 概率按上，0.1 按下…）
- 目标：让专家真实动作对应的概率尽可能大，等价于最小化交叉熵，就是最大似然。

举例子：专家在画面s按了「右」，我们希望模型输出 “右” 的概率接近 1，其他动作概率接近 0，用交叉熵计算差距。

2. **连续动作（比如机器人机械臂角度、赛车方向盘转角，动作是小数、连续数值）**

损失用**均方误差 MSE**

- 场景：动作是回归问题，输出一个连续数字 / 向量
- 网络直接输出动作数值（比如方向盘转角 0.75）
- MSE 计算：\((\text{模型输出动作} - \text{专家真实动作})^2\)，差值越大损失越高。

举例子：专家方向盘打 0.6，模型预测 0.2，差值 0.4，MSE 损失就是\(0.4^2=0.16\)，训练时缩小这个值。



> 通过 BC 学习得到的策略在和环境交互过程中不可能完全学成最优，只要存在一点偏差，就有可能导致下一个遇到的状态是在专家数据中没有见过的。此时，由于没有在此状态（或者比较相近的状态）下训练过，策略可能就会随机选择一个动作，这会导致下一个状态进一步偏离专家策略遇到的的数据分布。最终，该策略在真实环境下不能得到比较好的效果，这被称为行为克隆的**复合误差**（compounding error）问题



**占用**：智能体在整个轨迹里，会反复 “待在某个状态s、执行某个动作a”

**度量**：是一种概率分布，用来量化 “出现频次、长期访问概率”。

占用度量$\rho_\pi(s,a)$ = 智能体执行策略$\pi$时，\((s,a)\)这个状态 - 动作对出现的**长期访问频率**。
...
![image-20260326164012429](RL基本原理-img/image-20260326164012429.png)

* **维度灾难**

从单一智能体扩展到多个智能体，会引发`策略空间的指数级增长`，即所谓的“维度灾难” 
...
![image-20260420113852709](RL基本原理-img/image-20260420113852709.png)
